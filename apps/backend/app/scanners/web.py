...
async def scan_web(domain: str) -> list[CheckResult]:
    ...
    return [
      ...
      CheckResult(severity='medium',whyItMatters='This security signal affects business risk.',recommendedFix='Review the finding and take the recommended action.',category='Website security',title='HTTP status',status=http_status,plainEnglishSummary=http_message,evidence={'status_code':response.status_code if response else None})]
