...
async def scan_domain(request: ScanRequest):
    dns_checks,web_checks=await asyncio.gather(scan_dns(request.domain),scan_web(request.domain)); checks=dns_checks+web_checks; score=calculate_score(checks); result=ScanResponse(domain=request.domain,score=score,grade=grade(score),checks=checks)
